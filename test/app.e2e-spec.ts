...
process.env.JWT_SECRET = 'HFS&*(ioUY*&)^DF';
describe('App (e2e)', () => {
  ...
  describe('UserModule', () => {
    ...
    describe('GET /users', () => {
      ...
      it('Should fail because jwt user is not found', async () => {
        await userRepository.save([
          {
            username: 'legendary-seller',
            password: await bcrypt.hash('legendary-seller', 10),
            role: 'SELLER',
            id: 1,
          },
        ]);
      ...
      it('Should return users', async () => {
        await userRepository.save([
          ...
          {
            username: 'legendary-buyer',
            password: await bcrypt.hash('legendary-buyer', 10),
            role: 'BUYER',
            id: 2,
          },
        ]);
    ...
    describe('PUT /users/me', () => {
      ...
      it('Should update current login user', async () => {
        ...
        const { body, status } = await request(app.getHttpServer())
          .put('/users/me')
          .send({ username: 'legendary-seller-II' })
          .set({ Authorization: `Bearer ${token}` });
    ...
    describe('POST /auth/login', () => {
      ...
      it('Should login successful', async () => {
        await userRepository.save([
          {
            username: 'legendary-seller',
            password: await bcrypt.hash('legendary-seller-password', 10),
            role: 'SELLER',
            id: 1,
          },
        ]);
        const { body, status } = await request(app.getHttpServer())
          ...
          .send({
            username: 'legendary-seller',
            password: 'legendary-seller-password',
          });
